...
public:
		...
		: myData()
	{
		...
		//(right.myData.myHead->next) == right第一個node的地址，在2者size相同之前，每次ptr紀錄下一個node的地址
		for (nodePtr ptr = right.myData.myHead->next; size() != right.size(); ptr = ptr->next)
		{
			insert(end(), ptr->myVal); //每次將right當前的值加入當前串列的最尾端
		}
	}
	...
	list& operator=(const list& right)
	{
		if (this != &right)
		{
			nodePtr thisPtr = myData.myHead->next; //用來記錄this當前的node
			nodePtr rightPtr = right.myData.myHead->next; //用來記錄right當前的node
			for (; thisPtr != end() && rightPtr != right.end(); thisPtr = thisPtr->next, rightPtr = rightPtr->next) //在2種Ptr都還未指完各自list上的每個node之前
			{
				thisPtr->myVal = rightPtr->myVal; //每次將right內的值給this
			}
			...
			{
				for (thisPtr = thisPtr->prev; size() != right.size();) //此時thisPtr所指的node也需被刪除，故thisPtr先指回上一個node
				{
					erase(thisPtr->next); //在2串列的size相同之前，從thisPtr的下一個node開始全數刪除
				}
			}
		}
		return *this;

...
public:
	...
	deque(size_type count)
		: myData()
	{
		for (myData.mapSize = 8; 4 * myData.mapSize < count; myData.mapSize *= 2); //初始化，若初始capacity < count，則mapSize翻倍

		myData.map = new pointer[myData.mapSize](); //建立每格都是pointer的動態陣列
		for (size_t row = 0; row <= (count - 1) / 4; row++) //(count - 1) / 4 == 最後一個元素的所在列
		{
			myData.map[row] = new value_type[4](); //從0 ~ 該列都建立動態陣列
		}
	...
	deque(const deque& right)
		: myData()
	{
		size_t right_capacity = 4 * right.myData.mapSize; //right的陣列總格數
		for (myData.mapSize = 8; 4 * myData.mapSize < right.size(); myData.mapSize *= 2); //初始化，若初始this_capacity < right.size()，則mapSize翻倍

		myData.map = new pointer[myData.mapSize](); //建立每格都是pointer的動態陣列
		for (size_t row = 0; row <= (right.size() - 1) / 4; row++) //(right.size() - 1) / 4 == row最後建立動態陣列的位置
		{
			myData.map[row] = new value_type[4]();
		}

		//counter同時代表計數器和this當前的位置(從0開始)
		for (size_t counter = 0, pos = right.myData.myOff; counter < right.size(); counter++, (++pos) %= right_capacity)
		{
			myData.map[counter / 4][counter % 4] = right.myData.map[pos / 4][pos % 4];
		}
	...
	deque& operator=(const deque& right)
	{
		if (&right != this) // avoid self-assignment
		{
			for (; 4 * myData.mapSize < right.size(); doubleMapSize()); //初始化，若初始capacity < right.size()，則mapSize翻倍

			size_t this_capacity = 4 * myData.mapSize, right_capacity = 4 * right.myData.mapSize; //this和right的capacity
			size_t this_pos = myData.myOff, right_pos = right.myData.myOff; //this和right的pos
			for (size_t counter = 0; counter < right.size(); counter++, (++this_pos) %= this_capacity, (++right_pos) %= right_capacity)
			{
				if (myData.map[this_pos / 4] == nullptr) //若當前要assign的位置還未建立動態陣列
				{
					myData.map[this_pos / 4] = new value_type[4]();
				}

				myData.map[this_pos / 4][this_pos % 4] = right.myData.map[right_pos / 4][right_pos % 4];
			}
...
bool operator==(const deque< Ty >& left, const deque< Ty >& right)
{
	...
	for (int pos = left.size() - 1; pos >= 0; pos--)
	{
